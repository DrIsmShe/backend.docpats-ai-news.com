...
function validateTranslation(translated, locale) {
  ...
  /* Хвост очищаем от разметки перед проверкой.
     Шаблон статьи требует завершать её курсивной строкой-оговоркой, и
     последний символ текста — звёздочка. Проверка ждала знак препинания
     и браковала КАЖДЫЙ перевод: три попытки на каждый из четырёх языков,
     полтора часа платной генерации в корзину — на каждую статью.
     Настоящий обрыв на полуслове ловится по-прежнему: после снятия
     разметки проверяется тот символ, на котором текст правда кончился. */
  const tail = translated.body.trimEnd().replace(/[*_`~\s]+$/, "");
  if (!/[.!?:»)"'\]]$/.test(tail)) {
    throw new Error(
